...
function solveSudoku(board) {
    ...
    try {
        ...
        return {solved: false, status: "invalid", solution: null};
    }
    ...
    for (let row = 0; row < 9; row++) {
        for (let col = 0; col < 9; col++) {
            try {
		        ...
		        return {solved: false, status: "unsolvable", solution: null};
            }
        }
    }
    return {solved: true, status: "solved", solution: board};
}
...
export function generate_sudoku(difficulty) {
	...
	do {
		game = [];
		board = spawn(difficulty);
		// 0,0 0,1 0,2  1,0 1,1 1,2  2,0 2,1 2,3  0,3 0,4 0,5  1,3 1,4 1,5
		...
		console.log("Solved:", result, result.solved);
	} while (!result.solved)
